...
import pickle
import youtube_dl
from pf_funcion_prediccion import predict_musical_genre
...
raiz.title("Machine Learning para clasificación de audio")

raiz.iconbitmap("waveform.ico")
...
cuadrourl=Entry(miFrame, textvariable=url, width= 40)
cuadrourl.grid(row=0, column=1, padx=10, pady=10)
cuadrourl.config(fg="black", justify="left")

cuadrotitulo=Entry(miFrame, width= 40)
cuadrotitulo.grid(row=1, column=1, padx=10, pady=10)

cuadrogenero=Entry(miFrame)
cuadrogenero.grid(row=2, column=1, padx=10, pady=10)

urlLabel=Label(miFrame, text="""A ver que tal predice nuestro algoritmo el género de una canción.
Introduce la dirección url de Youtube:
Géneros: blues, classical, country, disco, hiphop, jazz, metal, pop, reggae, rock""")
urlLabel.grid(row=0, column=0, sticky="e", padx=10, pady=10)

tituloLabel=Label(miFrame, text="El título de la canción elegida es:")
tituloLabel.grid(row=1, column=0, sticky="e", padx=10, pady=10)

generoLabel=Label(miFrame, text="Esperamos... la predicción es:")
generoLabel.grid(row=2, column=0, sticky="e", padx=10, pady=10)

def codigoBoton():
    urlEntry=url.get()
    genero, titulo = predict_musical_genre(urlEntry)
    cuadrotitulo.insert(0, titulo)
    cuadrogenero.insert(0, genero)
